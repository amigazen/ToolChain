/*
 *	make.c - Do the actual making for make
 &	Modified 4/5/89 by LDH to compile under PDC.
 */
...
#ifndef AZTEC_C	   /* Aztec's preproc chokes on defined() */
#if defined(unix) || defined(pdc)
...
#endif
#ifdef pdc
	if ((fd = open(np->n_name, O_WRONLY)) < 0)
	    printf("%s: '%s' not touched - non-existant\n",
		   myname, np->n_name);
	close(fd);
#endif
