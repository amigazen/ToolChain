...
/*								    */
/*           Symbol table manipulation - November 2, 1989	    */
/*								    */
...
int  OpenIncl(), LineParts(), GetLine(), IsOperator();
int  ReadSymTab(), CountNest();
    ...
    it is closed first.  Returns TRUE if successful, FALSE if not.  */
{
    ...
    In.fd = NULL;
    return (FALSE);		/* We couldn't find it anywhere. */
}
	...
	For self-defining terms, DefLine2 is set to zero.	*/
{
	...
	for (s = numstart; *s; s++) {
	    if (result > 0xFFFFFFFL) {
		templong = result * radix;
		if (((templong / radix) != result) && !overflow) {
		    Error (loc, SizeErr);	/* Won't fit into a long word! */
		    overflow = TRUE;
		}
	    }
	    result *= radix;
